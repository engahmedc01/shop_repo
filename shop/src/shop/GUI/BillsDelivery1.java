 ...
 * @author Comu City
 */
public class BillsDelivery1 extends javax.swing.JPanel {

    /**
     * Creates new form BillsDelivery1
     */
    boolean isPurchased;
    ...
    public BillsDelivery1(Main_Frame myFrame, DataSourc data_source) {
    ...
    private void initComponents() {
        ...
        orderDet_table.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
...
void setCustomerOrders_tableSize() {
    ...
    public void fillCustomersTable() {
        ...
        Vector<Object> data = data_source.getCustomers(1, 0,seasonID,myFrame.firdage_id);
...
calculateOrdersTotalAmount();
    ...
    void search(String name) {
        ...
        Vector<Object> data = data_source.getCustomerOrders(name, 1, 0,seasonID);
        if (data != null) {
...
//        System.out.println("Number In Arabic : " + builder.toString());
    ...
    boolean validateForm() {
        double amount = Double.parseDouble(netPrice_TF.getText());
                    int order_id = Integer.parseInt(orderID_TF.getText());

            int customerID = data_source.getCustomerIdFromCustomerOrder(order_id);
            String customer_name = data_source.getcustomer_name(customerID);
                    if (!customer_name.startsWith("مشتروات") && !customer_name.startsWith("كريم_")) {
                        
        if (!data_source.vallidateSafeWithdrawal(amount)) {
            return false;
        }
                        }
        return true;
    ...
    private void delever_btnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_delever_btnActionPerformed
        ...
        if (validateForm()) {
            ...
            int customerID = data_source.getCustomerIdFromCustomerOrder(order_id);
            String customer_name = data_source.getcustomer_name(customerID);
            String orderDate = orderDate_TF.getText();
            double amount = Double.parseDouble(netPrice_TF.getText());

            if (!customer_name.startsWith("مشتروات") && !customer_name.startsWith("كريم_")) {
                data_source.outcomeTransaction(amount, notes, "ORDER_PAY", customerID, order_id, myFrame.firdage_id);
            }

            data_source.CustomerOrderDelvery(order_id, notes);
